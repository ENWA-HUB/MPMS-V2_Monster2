...
export function AccessControlPage(){
 ...
 const toggleValue=(m:string,v:string)=>{const s:Scope=access.scopes?.[m]||{mode:'OWN',values:[]},x=new Set(s.values||[]);x.has(v)?x.delete(v):x.add(v);setAccess({...access,scopes:{...access.scopes,[m]:{...s,values:Array.from(x)}})};
 ...
 return <><div className="page-title"><div><h1>Access Control</h1><p>Functional permission + data scope by user.</p></div><button className="budget-primary" disabled={busy} onClick={save}>{busy?'Saving…':'Save permissions'}</button></div>{msg&&<div className="budget-error">{msg}</div>}
