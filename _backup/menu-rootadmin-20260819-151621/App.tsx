...
export function App(){
 ...
 const visibleNav=useMemo(()=>access?nav.filter(x=>canView(navModule(x.key))):[],[access]);
 ...
 return <div className="shell">
   ...
   <main>
    ...
    <div className="page">
      ...
      {active==='risks'&&canView('RISKS')&&<RisksIssuesPage/>}
      {current?.path&&<DataPage title={current.label} path={current.path} columns={current.columns||[]}/>}
      {active==='budget'&&canView('BUDGET')&&<BudgetPage/>}
      {active==='budgetPlan'&&canView('BUDGET')&&<BudgetPlanningPage/>}
      {active==='suppliers'&&canView('SUPPLIERS')&&<SupplierPage/>}
      {active==='teamPerformance'&&<TeamPerformancePage/>}
      {active==='approvals'&&canView('APPROVALS')&&<ApprovalsPage/>}
      {active==='presentation'&&canView('EXECUTIVE')&&<PresentationPage/>}
      {active==='reports'&&canView('REPORTS')&&<ReportsPage/>}
