...
app.Use(async (context, next) =>
{
    ...
    var managerOnlyPrefixes = new[]
    {
        "/api/budget", "/api/suppliers", "/api/kpi", "/api/approvals",
        "/api/contracts", "/api/deliverables", "/api/change-requests",
        "/api/team/users", "/api/project-members", "/api/dashboard", "/api/risks", "/api/issues"
    };
    var managerOnly = managerOnlyPrefixes.Any(prefix => path.StartsWith(prefix, StringComparison.OrdinalIgnoreCase));
    var memberAllowedTeamSelf = path.Equals("/api/team/users", StringComparison.OrdinalIgnoreCase);
    if (managerOnly && !memberAllowedTeamSelf && !AuthService.IsManagerRole(auth.User.Role))
    {
        context.Response.StatusCode = StatusCodes.Status403Forbidden;
        await context.Response.WriteAsJsonAsync(new { message = "You do not have permission to access this management function." });
        return;
    }
    ...
    AuditActor.UserId = Convert.ToInt64(http.Items["AuthUserId"]);
    try
    {
        await next();
    }
...
app.MapPost("/api/auth/login", async (LoginRequest input, HttpContext http, AppDbContext db) =>
{
    ...
    var account = await db.AuthAccounts.FirstOrDefaultAsync(x => x.UserId == user.Id && x.IsEnabled);
    if (account is null || !AuthService.VerifyPassword(input.Password ?? "", account))
...
app.MapPost("/api/team/users/{id:long}/reset-login", async (long id, ResetPasswordRequest input, HttpContext http, AppDbContext db) =>
{
    ...
    var temporaryPassword = string.IsNullOrWhiteSpace(input.TemporaryPassword)
        ? $"Tmp@{Random.Shared.Next(100000, 999999)}!"
        : input.TemporaryPassword!;

    if (temporaryPassword.Length < 8)
        return Results.BadRequest(new { message = "Temporary password must contain at least 8 characters." });
...
app.MapGet("/api/projects", async (HttpContext http, AppDbContext db) =>
{
    ...
    var canManage = AuthService.IsManagerRole(
        Convert.ToString(http.Items["AuthUserRole"]));
    ...
    if (!canManage)
    {
        var projectIds = db.ProjectMembers
            .Where(m => m.UserId == userId && m.IsActive)
            .Select(m => m.ProjectId);

        q = q.Where(x => projectIds.Contains(x.Id));
    }
...
app.MapGet("/api/projects/form-options", async (AppDbContext db) =>
{
...
app.MapPost("/api/projects", async (Project input, HttpContext http, AppDbContext db) => { if(!AuthService.IsManagerRole(Convert.ToString(http.Items["AuthUserRole"]))) return Results.StatusCode(StatusCodes.Status403Forbidden); db.Projects.Add(input); await db.SaveChangesAsync(); return Results.Created($"/api/projects/{input.Id}", input); });
app.MapPut("/api/projects/{id:long}", async (long id, Project input, HttpContext http, AppDbContext db) =>
{
    if (!AuthService.IsManagerRole(Convert.ToString(http.Items["AuthUserRole"])))
        return Results.StatusCode(StatusCodes.Status403Forbidden);
    ...
    row.OrgUnitId = input.OrgUnitId;
    row.OwnerId = input.OwnerId;
    row.SponsorId = input.SponsorId;

    row.Status = input.Status;
    row.Priority = input.Priority;

    row.ProgressPct = input.ProgressPct;
    row.HealthScore = input.HealthScore;
    row.HealthStatus = input.HealthStatus;

    row.StartDate = input.StartDate;
    row.BaselineEndDate = input.BaselineEndDate;
    row.EndDate = input.EndDate;

    row.BudgetAmount = input.BudgetAmount;
...
app.MapGet("/api/team/users", async (HttpContext http, AppDbContext db) =>
{
    ...
    if (!canManage) q = q.Where(x => x.Id == userId);
    return Results.Ok(await q.OrderBy(x => x.Department).ThenBy(x => x.Name)
...
{
    var userId=Convert.ToInt64(http.Items["AuthUserId"]); var canManage=AuthService.IsManagerRole(Convert.ToString(http.Items["AuthUserRole"]));
    var q=db.Tasks.AsNoTracking().Include(x=>x.Project).Include(x=>x.Assignee).AsQueryable();
    if(!canManage) q=q.Where(x=>x.AssigneeId==userId || db.ProjectMembers.Any(m=>m.ProjectId==x.ProjectId && m.UserId==userId && m.IsActive));
    return Results.Ok(await q.Select(x => new { x.Id, x.ProjectId, x.AssigneeId, x.Code, x.Name, x.Status, x.Priority, x.ProgressPct, x.StartDate, x.DueDate, Project=x.Project!.Name, Assignee=x.Assignee != null ? x.Assignee.Name : null }).ToListAsync());
});
app.MapGet("/api/milestones", async (HttpContext http, AppDbContext db) =>
{
    ...
    if(!canManage) q=q.Where(x=>db.ProjectMembers.Any(m=>m.ProjectId==x.ProjectId && m.UserId==userId && m.IsActive));
    return Results.Ok(await q.Select(x => new { x.Id, x.ProjectId, x.Name, x.Status, x.DueDate, x.WeightPct, Project=x.Project!.Name }).ToListAsync());
});
...
app.MapGet("/api/budget-plan/overview", async (int? year, string? orgUnit, AppDbContext db) =>
{
    ...
    var auditUserIds = rows
        .SelectMany(x => new long?[] { x.CreatedByUserId, x.UpdatedByUserId })
        .Where(x => x.HasValue)
        .Select(x => x!.Value)
        .Distinct()
        .ToList();
    var auditUsers = await db.Users.AsNoTracking()
...
app.MapGet("/api/performance/overview", async (string? period, long? periodId, string? periodType, int? year, int? month, HttpContext http, AppDbContext db) =>
{
    var authUserId = Convert.ToInt64(http.Items["AuthUserId"]);
    var role = Convert.ToString(http.Items["AuthUserRole"]);
    ...
    if (!canManage) periodQuery = periodQuery.Where(x => x.UserId == authUserId);
    ...
    if (selected is null)
        return Results.Ok(new { periods = Array.Empty<object>(), selected = (object?)null, items = Array.Empty<object>(), teamScore = 0d, selfScore = 0d, managerScore = 0d, totalWeight = 0d, canManage });

    var items = await db.PerformanceItems.AsNoTracking().Include(x => x.Project)
        .Where(x => x.PerformancePeriodId == selected.Id).OrderBy(x => x.SourceRow).ToListAsync();
...
app.MapPost("/api/performance/periods", async (PerformancePeriod input, HttpContext http, AppDbContext db) =>
{
    ...
    else if (input.UserId.HasValue)
    {
        ...
        if (u != null) { input.EmployeeName = u.Name; if (string.IsNullOrWhiteSpace(input.Department)) input.Department = u.Department; }
    }
...
app.MapPut("/api/performance/periods/{id:long}", async (long id, PerformancePeriod input, HttpContext http, AppDbContext db) =>
{
    ...
    if (canManage)
    {
        ...
        if (input.UserId.HasValue)
        {
            ...
            if (u != null) x.EmployeeName = u.Name;
        }
...
app.MapPost("/api/performance/periods/{id:long}/lock", async (long id, HttpContext http, AppDbContext db) =>
{
    if (!AuthService.IsManagerRole(Convert.ToString(http.Items["AuthUserRole"]))) return Results.StatusCode(StatusCodes.Status403Forbidden);
    var x = await db.PerformancePeriods.FindAsync(id); if (x is null) return Results.NotFound();
    x.IsLocked = !x.IsLocked; x.LockedAt = x.IsLocked ? DateTime.UtcNow : null; x.Status = x.IsLocked ? "LOCKED" : "OPEN";
    await db.SaveChangesAsync(); return Results.Ok(new { x.Id, x.IsLocked, x.Status });
});
...
app.MapPut("/api/performance/items/{id:long}", async (long id, PerformanceItem input, HttpContext http, AppDbContext db) =>
{
    ...
    x.ProjectId = input.ProjectId; x.Strategy = input.Strategy; x.Function = input.Function; x.Plan = input.Plan;
...
{
    if (!AuthService.IsManagerRole(Convert.ToString(http.Items["AuthUserRole"]))) return Results.StatusCode(StatusCodes.Status403Forbidden);
    var result = await ExcelExchangeService.ExportBudgetAsync(db, year);
    return Results.File(result.Bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", result.FileName);
});

app.MapPost("/api/excel/budget/import", async (HttpRequest request, HttpContext http, AppDbContext db, bool dryRun = true, string mode = "merge") =>
{
    if (!AuthService.IsManagerRole(Convert.ToString(http.Items["AuthUserRole"]))) return Results.StatusCode(StatusCodes.Status403Forbidden);
    if (!request.HasFormContentType) return Results.BadRequest(new { message = "multipart/form-data required." });
    var form = await request.ReadFormAsync();
    ...
    if (!file.FileName.EndsWith(".xlsx", StringComparison.OrdinalIgnoreCase)) return Results.BadRequest(new { message = "Only .xlsx files are supported." });
    await using var stream = file.OpenReadStream();
    try { return Results.Ok(await ExcelExchangeService.ImportBudgetAsync(db, stream, file.FileName, dryRun, mode)); }
    catch (Exception ex) { return Results.BadRequest(new { message = ex.Message }); }
});
