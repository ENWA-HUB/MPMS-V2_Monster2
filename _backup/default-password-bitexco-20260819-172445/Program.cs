...
app.Use(async (context, next) =>
{
    ...
    if (mapped.Module is not null && mapped.Action is not null && !await RbacService.CanAsync(db, auth.User, mapped.Module, mapped.Action))
    {
...
app.MapPost("/api/budget-plan/years/{sourceYear:int}/clone",async(int sourceYear,int targetYear,HttpContext http,AppDbContext db)=>{var uid=Convert.ToInt64(http.Items["AuthUserId"]);var user=await db.Users.Include(x=>x.OrgUnit).FirstAsync(x=>x.Id==uid);if(!await RbacService.CanAsync(db,user,"BUDGET","FULL"))return Results.Forbid();if(sourceYear==targetYear)return Results.BadRequest(new{message="Target year must differ from source year."});if(await db.BudgetPlanItems.AnyAsync(x=>x.BudgetYear==targetYear))return Results.BadRequest(new{message=$"Budget plan {targetYear} already exists."});var source=await db.BudgetPlanItems.AsNoTracking().Where(x=>x.BudgetYear==sourceYear).ToListAsync();foreach(var x in source)db.BudgetPlanItems.Add(new BudgetPlanItem{BudgetYear=targetYear,OrgUnit=x.OrgUnit,SourceSheet=$"CLONED-{sourceYear}",SourceRow=x.SourceRow,ProjectId=x.ProjectId,Name=x.Name,Category=x.Category,Vendor=x.Vendor,PlannedAmount=x.PlannedAmount,Quantity=x.Quantity,UnitPrice=x.UnitPrice,PlannedMonth=x.PlannedMonth,MonthlyPlanJson=x.MonthlyPlanJson,Note=x.Note,Status="DRAFT"});await db.SaveChangesAsync();return Results.Created($"/api/budget-plan/overview?year={targetYear}",new{sourceYear,targetYear,items=source.Count});});
app.MapPost("/api/budget-plan",async(BudgetPlanItem input,HttpContext http,AppDbContext db)=>{var uid=Convert.ToInt64(http.Items["AuthUserId"]);var user=await db.Users.Include(x=>x.OrgUnit).FirstAsync(x=>x.Id==uid);if(!await RbacService.CanBudgetOrg(db,user,input.OrgUnit))return Results.Forbid();input.Id=0;input.Project=null;db.BudgetPlanItems.Add(input);await db.SaveChangesAsync();return Results.Created($"/api/budget-plan/{input.Id}",new{input.Id});});
app.MapPut("/api/budget-plan/{id:long}",async(long id,BudgetPlanItem input,HttpContext http,AppDbContext db)=>{var uid=Convert.ToInt64(http.Items["AuthUserId"]);var user=await db.Users.Include(x=>x.OrgUnit).FirstAsync(x=>x.Id==uid);if(!await RbacService.CanBudgetItem(db,user,id)||!await RbacService.CanBudgetOrg(db,user,input.OrgUnit))return Results.Forbid();var x=await db.BudgetPlanItems.FindAsync(id);if(x is null)return Results.NotFound();x.BudgetYear=input.BudgetYear;x.ProjectId=input.ProjectId;x.OrgUnit=input.OrgUnit;x.Name=input.Name;x.Category=input.Category;x.Vendor=input.Vendor;x.PlannedAmount=input.PlannedAmount;x.Quantity=input.Quantity;x.UnitPrice=input.UnitPrice;x.PlannedMonth=input.PlannedMonth;x.MonthlyPlanJson=input.MonthlyPlanJson;x.Note=input.Note;x.Status=input.Status;await db.SaveChangesAsync();return Results.Ok(new{x.Id});});
app.MapDelete("/api/budget-plan/{id:long}",async(long id,HttpContext http,AppDbContext db)=>{var uid=Convert.ToInt64(http.Items["AuthUserId"]);var user=await db.Users.Include(x=>x.OrgUnit).FirstAsync(x=>x.Id==uid);if(!await RbacService.CanBudgetItem(db,user,id))return Results.Forbid();var x=await db.BudgetPlanItems.FindAsync(id);if(x is null)return Results.NotFound();db.BudgetPlanItems.Remove(x);await db.SaveChangesAsync();return Results.NoContent();});
