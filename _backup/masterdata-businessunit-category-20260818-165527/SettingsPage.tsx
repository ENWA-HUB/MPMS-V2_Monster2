...
import {BriefcaseBusiness,Building2,UsersRound,Plus,Edit3,Trash2,X,KeyRound,RotateCcw,Power,PowerOff} from 'lucide-react';
import {getJson,postJson,putJson,deleteJson} from '../lib/api';
import { TeamManagementPage } from './TeamManagementPage';
import { KpiPage } from './KpiPage';
type Tab='portfolios'|'orgUnits'|'members'|'projectTeam'|'supplierKpi';
...
export function SettingsPage(){
 ...
 const path=tab==='portfolios'?'portfolios':tab==='orgUnits'?'org-units':'team-members';
 ...
 const fresh=()=>tab==='portfolios'?{orgUnitId:opts.orgUnits[0]?.id||'',code:'',name:'',description:'',ownerId:null,status:'ACTIVE'}:tab==='orgUnits'?{code:'',name:'',type:'SBU',status:'ACTIVE'}:{orgUnitId:opts.orgUnits[0]?.id||'',name:'',email:'',jobTitle:'',department:'',role:'MEMBER',status:'ACTIVE'};
 ...
 const save=async(e:any)=>{
  ...
  try{
    ...
    if(tab==='members'){
      payload={
        orgUnitId:Number(edit.orgUnitId),
        name:String(edit.name||'').trim(),
        email:String(edit.email||'').trim().toLowerCase(),
        jobTitle:String(edit.jobTitle||'').trim(),
        department:String(edit.department||'').trim(),
        role:String(edit.role||'MEMBER'),
 ...
 return <><div className="page-title"><div><h1>Settings</h1><p>Portfolio, Business Unit, Team Member and access master data</p></div>{tab!=='projectTeam'&&tab!=='supplierKpi'&&<button className="budget-primary" onClick={()=>start()}><Plus size={16}/> New</button>}</div>
 ...
 <div className="settings-tabs">
  ...
  <button className={tab==='members'?'active':''} onClick={()=>setTab('members')}><UsersRound/>Team Members</button>
 ...
 {tab==='orgUnits'&&<label>Type<select value={edit.type||'SBU'} onChange={e=>setEdit({...edit,type:e.target.value})}><option>SBU</option><option>DEPARTMENT</option><option>DIVISION</option><option>TEAM</option></select></label>}
 ...
 </div>{tab==='portfolios'&&<label>Description<textarea rows={4} value={edit.description||''} onChange={e=>setEdit({...edit,description:e.target.value})}/></label>}<div className="budget-modal-actions"><button type="button" className="secondary" onClick={()=>setOpen(false)}>Cancel</button><button className="budget-primary">Save</button></div></form></div></div>}</>;
}
