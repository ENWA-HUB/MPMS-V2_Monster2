...
export function App(){
 ...
 const current=visibleNav.find(x=>x.key===active) || visibleNav[0];
 if(!visibleNav.some(x=>x.key===active))setTimeout(()=>setActive(visibleNav[0]?.key||'dashboard'),0);
 ...
 return <div className="shell">
   <aside className={`sidebar ${mobile?'open':''}`}>
    ...
    {canManage&&<><div className="nav-section">SYSTEM</div><nav><button onClick={()=>setActive('settings')} className={active==='settings'?'active':''}><Settings size={18}/><span>Administration</span></button></nav></>}
   ...
   <main>
    ...
    <div className="page">
      ...
      {active==='kpi'&&canManage&&<KpiPage/>}
      {active==='projectTeam'&&canManage&&<TeamManagementPage/>}
      ...
      {active==='settings'&&canManage&&<Placeholder title="Administration" text="Organization, users, authentication, KPI criteria, master data, RBAC and audit-log administration."/>}
    </div>
   </main>
 </div>
}
