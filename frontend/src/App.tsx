...
export function App(){
 ...
 const systemKeys = ['settings','accessControl'];
 ...
 return <div className="shell">
   <aside className={`sidebar ${mobile?'open':''}`}>
    ...
    <><div className="nav-section">SYSTEM</div><nav><button onClick={()=>setActive('settings')} className={active==='settings'?'active':''}><Settings size={18}/><span>Settings</span></button><button onClick={()=>setActive('accessControl')} className={active==='accessControl'?'active':''}><Settings size={18}/><span>Access Control</span></button></nav></>
   ...
   <main>
    ...
    <div className="page">
      ...
      {active==='settings'&&(canManage?<SettingsPage/>:denied('ADMINISTRATION'))}
      {active==='accessControl'&&(canManage?<AccessControlPage/>:denied('ADMINISTRATION'))}
    </div>
   </main>
 </div>
}
