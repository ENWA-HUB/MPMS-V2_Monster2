...
export function TasksMilestonesPage(){
 ...
 const [task,setTask]=useState({projectId:'',code:'',name:'',description:'',assigneeId:'',status:'TODO',priority:'MEDIUM',startDate:'',dueDate:'',progressPct:'0',estimatedHours:'0'});
 ...
 const editTask=async(t:Task)=>{const name=prompt('Task name',t.name);if(name===null)return;const newStatus=prompt('Status',t.status);if(newStatus===null)return;const progress=prompt('Progress %',String(t.progressPct));if(progress===null)return;try{await putJson(`/tasks/${t.id}`,{...t,name,status:newStatus,progressPct:Number(progress)});await load()}catch(e){setError(String(e))}};
 const editMilestone=async(m:Milestone)=>{const name=prompt('Milestone name',m.name);if(name===null)return;const newStatus=prompt('Status',m.status);if(newStatus===null)return;try{await putJson(`/milestones/${m.id}`,{...m,name,status:newStatus});await load()}catch(e){setError(String(e))}};
